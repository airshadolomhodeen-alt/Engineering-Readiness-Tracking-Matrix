...
def load_data():
    ...
    try:
        ...
        df_master = pd.DataFrame(columns=["Project No.", "Title", "Sector", "Category", "Estimate Amount"])
    ...
    try:
        ...
        df_proposals = pd.concat(proposals_list, ignore_index=True) if proposals_list else pd.DataFrame()
    except Exception:
        df_proposals = pd.DataFrame(columns=["Project No.", "Title", "Sector", "Category", "Estimate Amount", "Funding Source"])
...
def clean_and_standardize(df, source_type="Masterplan"):
    if df.empty:
        return pd.DataFrame(columns=["Project_No", "Title", "Sector", "Category", "Estimate_Amount", "Source", "Funding_Source"])
    
    df = df.copy()
    col_map = {}
    for col in df.columns:
        col_lower = str(col).lower().strip()
        if 'project' in col_lower and ('no' in col_lower or 'code' in col_lower or '#' in col_lower):
        ...
        elif any(k in col_lower for k in ['cost', 'amount', 'estimate', 'budget']):
    ...
    return df[['Project_No', 'Title', 'Sector', 'Category', 'Estimate_Amount', 'Source', 'Funding_Source']]
...
df_combined = pd.concat([df_master_clean, df_proposals_clean], ignore_index=True)

# Automatic Committee Mapping
def assign_committee(row):
    sec = str(row['Sector']).lower()
    title = str(row['Title']).lower()
    if 'transport' in sec or 'road' in sec or 'bridge' in sec or 'transport' in title:
        return 'Transportation Sub-Committee'
    elif 'international' in sec or 'foreign' in sec or 'oda' in sec or 'trade' in sec:
        return 'International Development Sectoral Working Group (IDSWG)'
    elif 'infrastructure' in sec or 'energy' in sec or 'water' in sec or 'construct' in sec:
...
selected_committee = st.sidebar.selectbox("Select Committee Scope", committee_options)

st.sidebar.markdown("---")
st.sidebar.subheader("🔍 Global Filters")

df_filtered = df_combined[df_combined['Assigned_Committee'] == selected_committee].copy()
...
funding_sources = sorted(df_filtered['Funding_Source'].dropna().unique().tolist())
selected_funding = st.sidebar.multiselect("Funding Sources", funding_sources, default=funding_sources)

min_budget = float(df_filtered['Estimate_Amount'].min()) if not df_filtered.empty else 0.0
max_budget = float(df_filtered['Estimate_Amount'].max()) if not df_filtered.empty else 100000000.0
if min_budget == max_budget:
    max_budget += 1.0

budget_range = st.sidebar.slider("Budget Range (PHP)", min_value=min_budget, max_value=max_budget, value=(min_budget, max_budget))
...
avg_project_cost = df_filtered['Estimate_Amount'].mean() if active_proposals > 0 else 0
...
with tab1:
    ...
    with col_a:
        if not df_filtered.empty:
            fig_sector = px.pie(df_filtered, names='Sector', values='Estimate_Amount', title="Budget Allocation by Sector", hole=0.4)
    ...
    with col_b:
        if not df_filtered.empty:
            fig_bar = px.bar(
                ...
                title="Masterplan vs. New Proposals by Category", barmode='group'
            )
...
with tab2:
    ...
    growth_multiplier = st.slider("Regional Economic Growth Multiplier", 1.0, 2.5, 1.40, 0.05)
    discount_rate = st.slider("Discount Rate (%)", 3.0, 12.0, 8.0, 0.5) / 100.0
    
    if not df_filtered.empty:
        sim_df = df_filtered.copy()
        sim_df['Projected_Revenue'] = sim_df['Estimate_Amount'] * growth_multiplier
        sim_df['Estimated_NPV'] = (sim_df['Projected_Revenue'] / (1 + discount_rate)) - sim_df['Estimate_Amount']
        sim_df['BCR'] = sim_df['Projected_Revenue'] / sim_df['Estimate_Amount'].replace(0, 1)
        
        fig_econ = px.scatter(
            sim_df, x='Estimate_Amount', y='Projected_Revenue', size='BCR', color='Sector',
            hover_name='Title', title="Project Cost vs. Projected Revenue & BCR Scaling"
        )
...
with tab3:
    st.subheader("Predictive Modeling & Principal Component Analysis (PCA)")
    st.markdown("Multivariate clustering of project proposals to identify high-priority strategic investments.")
    ...
    if len(df_filtered) > 3:
        ...
        fig_pca = px.scatter(pca_df, x='PC1', y='PC2', color='Sector', hover_name='Title', title="PCA 2D Cluster Map")
        st.plotly_chart(fig_pca, use_container_width=True)
    else:
        st.warning("Insufficient data points for PCA clustering under current filters.")

with tab4:
    st.subheader("Decision-Making Matrix Table")
    st.markdown("Examine detailed project attributes and export selected subsets for official committee resolutions.")
    
    if not df_filtered.empty:
        display_df = df_filtered[['Project_No', 'Title', 'Sector', 'Category', 'Estimate_Amount', 'Source', 'Funding_Source', 'Assigned_Committee']]
